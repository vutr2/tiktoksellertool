-- Paid credits no longer expire (owner decision, 2026-09-15).
--
-- SPEC §6 had unused subscription credits expiring at the end of each billing
-- month. Apple Guideline 3.1.1 says credits a customer paid for may not expire,
-- so the rule now splits in two:
--
--   * Credits keep their value indefinitely once paid for, both the monthly
--     plan allowance and top-ups.
--   * Feature access still ends with the subscription. billing.ts decides that
--     from the `subscriptions` table (status + expires_at), never from the
--     credit balance, so this migration does not touch it.
--
-- The free trial still expires: it was never paid for, and Guideline 3.1.1
-- concerns purchased content.
...
-- Plan grants that have not expired yet lose their deadline.
--
-- An already-expired grant is left alone on purpose: reversing it would mint
-- credits out of a rule change rather than a payment. No production data exists
