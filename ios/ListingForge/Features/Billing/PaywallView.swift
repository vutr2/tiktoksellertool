        ...
        NavigationStack {
            List {
                ...
                Section("Subscriptions") {
                    ForEach(billing.products.filter { $0.type == .autoRenewable }) { product in
                ...
                Section {
                    Button("Restore Purchases") { Task { await billing.restore() } }.disabled(billing.isBusy)
                    Button("Refresh plans") { Task { await billing.refresh() } }.disabled(billing.isBusy)
                    ...
                    if let message = billing.message { Text(message).font(.footnote).accessibilityLabel(message) }
